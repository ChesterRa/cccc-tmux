...
| **PeerB** | 主要な実行者の1人、PeerAと対等に協力；`none`でシングルPeerモード | はい* |
...
> **シングルPeerモード**：PeerBを`none`に設定すると、PeerAが単独で動作し、Foreman、セルフチェック、キープアライブなど完全なインフラサポートを利用できます。

### マルチプラットフォームブリッジ

- **Telegram / Slack / Discord / WeCom**：オプションで接続し、チームが普段使う場所に作業を持ち込む
- **双方向通信**：チャットでコマンド送信、ステータス受信、RFD承認（WeComはアウトバウンドのみ[beta]）
...
```bash
# 基本リセット：state/mailbox/logs/workをクリアし、タスクファイルを削除
cccc reset

# アーカイブモード：タスクをタイムスタンプ付きアーカイブに移動してからクリア
cccc reset --archive
```
...
- タスクファイルをリセットして計画をやり直す場合

> **注意**：オーケストレーターが実行中の場合、確認が求められます。先に `cccc kill` を実行することをお勧めします。

### シングルPeerモードは使えますか？

はい！SetupパネルでPeerBを`none`に設定すると、PeerAが単独で動作し、完全なインフラサポートを利用できます：
- Foremanの定期チェック
- セルフチェックリマインダー
- キープアライブ
- ccontext Context追跡
